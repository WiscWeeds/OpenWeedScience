...
- Statistics
- ANOVA
thumbnail: https://cropwatch.unl.edu/2017-CW-News/2017-images/weeds/soybeans-Rowel-pre-AJ-F3.jpg
...
After running the codes above, the data should appear as *control* (“control” was chosen as the name of the dataset; you could have called it “data” or something else).
...
The dataset contains 2 years (2013 and 2014), 5 herbicides (A, B, C, D, E), 3 reps (1, 2 ,3), and visual control of waterhemp (converted to proportions [0.001-0.999] already).


# Fligner test of homogeneity of variances 

Performs Fligner's test of the null that the variances amongst herbicide treatments are the same. The test is needed to check a ANOVA assumption.
...
```{r}
fligner.test(control$control, control$herbicide)
```


Results show p-value = 0.6086 thus the null hypothesis is accepted indicating that variances amongst herbicide treatments are the same.
...
```{r}
model <- glmmTMB(control ~ herbicide + (1|rep), beta_family(link = "logit"), data=control)
```
...
- Use a non-parametric Fligner Test for checking homogeneity of variances 

- Use a Non-parametric *glmmTMB* function in the model
